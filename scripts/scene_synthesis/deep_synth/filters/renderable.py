...
def renderable_room_filter(height_cap=4, length_cap=6, width_cap=6):
    def room_criteria(room, house):
        ...
        if not os.path.isfile(f"{data_dir}/3d_front/room/{room.house_id}/{room.modelId}f.obj"):
            return False
        if not os.path.isfile(f"{data_dir}/3d_front/room/{room.house_id}/{room.modelId}w.obj"):
            return False
